...
Pure, mostly Obsidian-agnostic utilities shared across the plugin: time-block computations, markdown text manipulation, time/moment helpers, iCal conversion, DOM helpers, color/contrast, and small functional helpers. This is the home for logic that should be unit-testable in isolation.

## Key Files

| File                                                                                                                                                                                                                                    | Description                                                                                                                      |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `time-block-utils.ts`                                                                                                                                                                                                                   | Time-block math + rendering: end times, duration guards, keys, copying, day truncation, and nested schedule rendering.           |
...
| `ical.ts`                                                                                                                                                                                                                               | Convert iCal events to time blocks for a date range (`icalEventToTimeBlocksForRange`, `icalEventToTimeBlock`, `canHappenAfter`). |
...
| `clock.ts`, `editor.ts`, `error.ts`, `ellipsis.ts`, `id.ts`, `to-spliced.ts`, `store.ts`, `performance.ts`, `notify-about-started-time-blocks.ts`, `handle-active-leaf-change.ts`, `create-environment-hooks.ts`, `create-error-url.ts` | Focused single-purpose helpers (clocks, editor positions, error formatting, ids, array ops, notifications, etc.).                |
...
- `time-block-utils.ts` rendering (`toRenderableMarkdown`) feeds `ui/components/rendered-markdown.svelte`; preserve time ranges and divider behavior and update `tests/integration/task-views.test.ts`.
...
- `src/time-block-types.ts`, `src/types.ts`, `src/regexp.ts`, `src/parser/`
