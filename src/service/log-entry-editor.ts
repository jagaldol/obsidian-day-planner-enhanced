...
export class LogEntryEditor {
  ...
  deleteClock = (location: ClockLocation, locator: LogEntryLocator) =>
    editYaml(
      this.targetFor(location),
      requireProps((props) => deleteLogEntry(props, locator)),
    );

  editClock = (
    location: ClockLocation,
    args: LogEntryLocator & { patch: { start?: string; end?: string } },
  ) =>
    editYaml(
      this.targetFor(location),
